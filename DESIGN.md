...
- **Overscroll-to-close**: when already at bottom, swipe up >60pt dismisses by continuing off the top of the screen; closing duration scales with swipe velocity (clamped 100–280ms), so a fast fling closes quicker than a slow drag-release. The X button and hardware back button instead slide back down (mirroring the open animation) at a fixed 220ms, since there's no swipe velocity to match and it's a visually distinct close path from the swipe gesture.
